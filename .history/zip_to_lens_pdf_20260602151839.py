...
def extract_page_number_from_footer(img: Image.Image, lang: str = "eng") -> Optional[int]:
    ...
    Asume que el número está en la zona inferior de la imagen (últimos 10-15% de altura).
    Busca números en esa región y devuelve el primero encontrado.
    """
    try:
        rgb = img.convert("RGB")
        w, h = rgb.size
        
        # Crop: últimos 15% de la altura
        footer_height = int(h * 0.15)
        footer_box = (0, h - footer_height, w, h)
        footer_crop = rgb.crop(footer_box)
        
        # OCR en el pie de página
        ocr_text = pytesseract.image_to_string(footer_crop, lang=lang)
        
        # Buscar número: toma el primer número encontrado (de izquierda a derecha)
