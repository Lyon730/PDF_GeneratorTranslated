...
# Optional: Advanced translation and OCR
try:
    import cv2
except Exception:
    cv2 = None
...
def extract_page_number_from_footer(img: Image.Image, lang: str = "eng") -> Optional[int]:
    """Extrae el número de página del pie de página usando OCR.

    Busca en la parte inferior de la imagen y devuelve el número más probable.
    """
    try:
        rgb = img.convert("RGB")
        w, h = rgb.size

        footer_height = max(40, int(h * 0.22))
        footer_top = max(0, h - footer_height)
        footer_crop = rgb.crop((0, footer_top, w, h))

        data = pytesseract.image_to_data(footer_crop, lang=lang, output_type=pytesseract.Output.DICT)
        ...
        for i in range(n):
            ...
            try:
                ...
                pass
            top = int(data["top"][i]) if data.get("top") else 0
            bottom = top + int(data["height"][i]) if data.get("height") else top

            # Prefiere tokens completamente numéricos o con dígitos relevantes
            match = re.search(r"(\d+)", text)
            if not match:
                continue

            num = int(match.group(1))
            # Score by proximity to bottom and OCR confidence
            score = (bottom, conf)
            found.append((num, score[0], score[1]))

        if found:
            found.sort(key=lambda x: (-x[1], -x[2]))
            return found[0][0]

        # Fallback: OCR en la región inferior como texto completo
        raw = pytesseract.image_to_string(footer_crop, lang=lang)
...
def try_translate_en_to_es(text: str) -> str:
    ...
    # Intenta DeepL primero (mejor calidad)
    if deepl:
        try:
            translator = deepl.Translator("")  # Clave vacía usa API free
            result = translator.translate_text(text, source_lang="EN", target_lang="ES")
            if result:
                return result.text
        except Exception:
            pass

    # Fallback a GoogleTranslator
    try:
...
def ocr_with_easyocr(img_rgb: Image.Image, lang: str = "en") -> list[LineBox]:
    """Detecta texto con EasyOCR. Retorna lista de LineBox.
    ...
    try:
        ...
        cv_img = cv2.cvtColor(
            cv2.UMat(
                __import__("numpy").array(img_rgb)
            ).get(),
            cv2.COLOR_RGB2BGR
        ) if cv2 else None
        
        if cv_img is None:
            # Fallback si cv2 no está disponible: convertir PIL a array
            import numpy as np
            cv_img = cv2.cvtColor(
                np.array(img_rgb),
                cv2.COLOR_RGB2BGR
        ...
        for detection in results:
            pts = detection[0]  # [[x1,y1], [x2,y2], [x3,y3], [x4,y4]]
            text = detection[1]
            conf = detection[2]
...
def inpaint_text_region(
    ...
    """Usa inpainting para borrar regiones de texto y rellenarlas de manera inteligente.
    ...
    try:
        ...
        # Dilatar la máscara para cubrir más área
        if dilation > 0:
...
def convert_pdf_to_translated_pdf(
    pdf_path: Path,
    out_dir: Path,
    *,
    ...
    if not is_tesseract_available():
        raise RuntimeError("No se encontró Tesseract OCR. Instálalo o indica --tesseract-cmd.")

    try:
        import fitz
    except ImportError as exc:
        raise RuntimeError(
            "Para traducir PDFs se requiere pymupdf. Instálalo con: pip install pymupdf"
        ) from exc
    ...
    for page in range(n_pages):
        ...
        translated_img, _ = draw_translated_overlay(
            img, lang_ocr=ocr_lang, ocr_max_width=ocr_max_width
        )
        out_path = translated_dir / f"{page + 1:03d}.png"
        translated_img.save(out_path, format="PNG", optimize=True)

    translated_images = sorted(translated_dir.glob("*.png"), key=lambda p: natural_sort_key(p.name))
    pdf_es_path = pdf_dir / f"{sanitize_filename(pdf_path.stem)} - ES.pdf"
    build_pdf_from_images(translated_images, pdf_path=pdf_es_path, title=f"{pdf_path.stem} - ES")
    return pdf_es_path
...
def extract_images_from_zip(zip_path: Path, out_dir: Path) -> list[Path]:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        for name in names:
            ...
            if target.exists():
                ...
                count = 1
                while (out_dir / f"{stem}_{count}{suffix}").exists():
                    count += 1
                target = out_dir / f"{stem}_{count}{suffix}"

            with zf.open(name) as src, open(target, "wb") as dst:
                shutil.copyfileobj(src, dst)
            extracted.append((target, basename))
    ...
    for index, (img_path, basename) in enumerate(extracted):
        page_num = extract_page_number_from_filename(basename)
        if page_num is None:
            try:
        ...
        original_order[img_path] = index

    if any(pn is not None for pn in page_numbers.values()):
        extracted.sort(
            key=lambda item: (
                page_numbers[item[0]] is None,
                page_numbers[item[0]] if page_numbers[item[0]] is not None else original_order[item[0]],
                natural_sort_key(item[1]),
            )
        )

    return [img_path for img_path, _basename in extracted]
        ...
        if generate_es:
            ...
            if jobs:
                if max_workers == 1:
                    ...
                    # Ensure order in PDF: by filename prefix index (natural sort)
