...
importlib.reload(zip_to_lens_pdf)
...
with col_left:
    ...
    uploaded_pdf = st.file_uploader("Sube un PDF para traducir", type=["pdf"], accept_multiple_files=False)

    st.caption(
        "Genera 2 PDFs desde un ZIP de imágenes: uno original (EN) y otro traducido (ES). "
        "Si subes un PDF, solo se traducirá a ES y se descargará el PDF traducido."
    )
...
with col_right:
    ...
    run_zip = st.button("Generar PDFs", type="primary", use_container_width=True, disabled=uploaded is None)
    run_pdf = st.button("Traducir PDF", use_container_width=True, disabled=uploaded_pdf is None)
    cancel = st.button("Cancelar", use_container_width=True, disabled=(uploaded is None and uploaded_pdf is None))

    if cancel:
        st.session_state.cancel_requested = True

    if uploaded is None and uploaded_pdf is None:
